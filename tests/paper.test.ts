...
import { PaperExchange, TAKER_FEE, type Quote } from "../src/exchange/paper.js";
import type { Intent } from "../src/agent/schema.js";

// Tests that don't care about the spread pass a single price as both sides.
function q(price: number, spread = 0): Quote {
  return { bid: price - spread / 2, ask: price + spread / 2, mid: price };
}
...
test("flat round-trip: equity falls by exactly 2 * (sizeUsd * fee)", () => {
  ...
  const open = ex.fill(buy(), q(100), "open-1");
  ...
  const closed = ex.fill(close(), q(100), "close-1");
...
test("5% favourable move on 20% notional → expected net PnL", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy(), q(100), "open-1");
  // Move 5% up — exit at 105.
  const closed = ex.fill(close(), q(105), "close-1");
...
test("feePaidUsd reflects notional, not |PnL| (regression)", () => {
  ...
  ex2.fill(buy(), q(100), "open2");
  const lost = ex2.fill(close(), q(50), "close2");
...
test("checkExits triggers on intra-bar stop wick (regression for C2)", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ stop_loss_pct: 1 }), q(100), "open");
...
test("checkExits triggers on intra-bar take-profit reach", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ stop_loss_pct: 5, take_profit_pct: 2 }), q(100), "open");
...
test("checkExits returns null inside the corridor", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ stop_loss_pct: 1, take_profit_pct: 2 }), q(100), "open");
...
test("when stop and TP both fall inside the same bar, stop wins (conservative)", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ stop_loss_pct: 1, take_profit_pct: 1 }), q(100), "open");
...
test("sizePct on closed trade uses entry-time equity (regression for H17)", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ size_pct_of_equity: 20 }), q(100), "open");
  // Take a big loss so post-fill equity diverges sharply from entry-time.
  const lost = ex.fill(close(), q(50), "close");
...
test("getHighWater rises with equity but does not fall when equity drops (regression for C6)", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ stop_loss_pct: 5, take_profit_pct: 50 }), q(100), "open-1");
  ...
  ex.fill(buy({ stop_loss_pct: 5, take_profit_pct: 100 }), q(200), "open-2");
...
test("closeAll closes every open position at the supplied price (regression for C7)", () => {
  ...
  ex.fill(buy({ symbol: "ETH/USDT" }), q(50), "open-eth");
...
test("BUY fills at ask, qty derived from ask (N4)", () => {
  const ex = new PaperExchange(10_000);
  // bid=99.9, ask=100.1, mid=100. A market BUY must cross to the ask.
  const open = ex.fill(buy(), { bid: 99.9, ask: 100.1, mid: 100 }, "open");
  assert.equal(open.fillPrice, 100.1);
  assert.equal(open.position?.entryPrice, 100.1);
  // notional = 20% * 10_000 = 2000; qty = 2000 / 100.1.
  assert.ok(Math.abs((open.position?.qty ?? 0) - 2000 / 100.1) < 1e-9);
});

test("SELL fills at bid, qty derived from bid (N4)", () => {
  const ex = new PaperExchange(10_000);
  const sell: Intent = {
    action: "SELL",
    symbol: "BTC/USDT",
  ...
  const open = ex.fill(sell, { bid: 99.9, ask: 100.1, mid: 100 }, "open-short");
  assert.equal(open.fillPrice, 99.9);
  assert.equal(open.position?.entryPrice, 99.9);
  assert.ok(Math.abs((open.position?.qty ?? 0) - 2000 / 99.9) < 1e-9);
});

test("CLOSE long sells into the bid; CLOSE short buys at the ask (N4)", () => {
  // Long open at ask=100.1 then closed at bid=104.9 — exit < mid by half spread.
  const longEx = new PaperExchange(10_000);
  longEx.fill(buy(), { bid: 99.9, ask: 100.1, mid: 100 }, "open");
  const longClose = longEx.fill(
    close(),
    { bid: 104.9, ask: 105.1, mid: 105 },
    "close",
  );
  assert.equal(longClose.fillPrice, 104.9);
  assert.equal(longClose.trade?.exitPrice, 104.9);

  // Short open at bid=100.1 (mid=100.2) then closed at ask=95.3.
  const shortEx = new PaperExchange(10_000);
  const sell: Intent = {
    action: "SELL",
    symbol: "BTC/USDT",
    size_pct_of_equity: 20,
  ...
  shortEx.fill(sell, { bid: 100.1, ask: 100.3, mid: 100.2 }, "open-short");
  const shortClose = shortEx.fill(
    close(),
    { bid: 95.1, ask: 95.3, mid: 95.2 },
    "close-short",
  );
  assert.equal(shortClose.fillPrice, 95.3);
...
test("exportState/restoreState round-trip preserves equity, HWM, day, positions, history", () => {
  const ex = new PaperExchange(10_000);
  ex.fill(buy({ symbol: "BTC/USDT" }), q(100), "open-btc");
  ex.fill(buy({ symbol: "ETH/USDT", size_pct_of_equity: 10 }), q(50), "open-eth");
  // Close one to populate dailyPnlHistory via getEquity-triggered rollover later.
  ex.fill(close(), q(110), "close-btc");
  const beforeEquity = ex.getEquity();
  const beforeHwm = ex.getHighWater();
  const snap = ex.exportState();
  assert.equal(snap.positions.length, 1);
  assert.equal(snap.positions[0]?.symbol, "ETH/USDT");

  const restored = new PaperExchange(1); // wrong initial — restore must overwrite
  restored.restoreState(snap);
  assert.equal(restored.getEquity(), beforeEquity);
  assert.equal(restored.getHighWater(), beforeHwm);
  assert.equal(restored.getOpenPosition("ETH/USDT")?.entryPrice, 50);
  assert.equal(restored.getOpenPosition("BTC/USDT"), undefined);
});
